...
Radix sort will operate on nn dd-digit numbers where each digit can be one of at most bb different values (since bb is the base being used). For example, in base 10, a digit can be 0,1,2,3,4,5,6,7,8, \text{or } 90,1,2,3,4,5,6,7,8,or 9.

Radix sort uses counting sort on each digit. Each pass over nn dd-digit numbers will take O(n + b)O(n+b) time, and there are dd passes total. Therefore, the total running time of radix sort is O(d(n+b))O(d(n+b)). When dd is a constant and bb isn't much larger than nn (in other words, b = O(n)b=O(n)), then radix sort takes linear time.
